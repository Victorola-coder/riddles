...
import { Search, HelpCircle, Plus, Edit, Trash2, Eye, EyeOff } from 'lucide-react';
...
export default function RiddlesPage() {
  ...
  const [editingRiddle, setEditingRiddle] = useState<any>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useAdminRiddles(page, PAGE_SIZE, debouncedSearch || undefined, difficultyFilter);
  ...
  const handleToggleActive = async (riddle: any) => {
    updateRiddle.mutate({
      id: riddle.id,
      ...
      <div className="bg-[#161616] rounded-lg border border-[#FFFFFF1A] overflow-hidden">
        {riddles.length === 0 ? (
          <div className="py-16 px-6 text-center">
            ...
            <p className="text-lg font-medium text-white mb-2">No riddles found</p>
                      ...
                      <td className="px-6 py-4">
                        <div className="max-w-md">
                          ...
                          <p className="text-gray-400 text-xs mt-1">
                            Answer: {Array.isArray(riddle.answer) ? riddle.answer.join(', ') : riddle.answer}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
            ...
            {totalRiddles > 0 && (
              <div className="mt-4 p-4 border-t border-[#FFFFFF1A] flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <p className="text-sm text-gray-400">
                  Showing {((page - 1) * PAGE_SIZE) + 1}–{Math.min(page * PAGE_SIZE, totalRiddles)} of {totalRiddles} riddles
                </p>
                <div className="flex items-center gap-2">
                  <button
      ...
      >
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-300 mb-1">Hint 1 (First Letter)</label>
              <Input
                type="text"
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1">Hint 2 (Word Length)</label>
              <Input
                type="text"
